...
public class CountriesService : ICountriesService
{
    ...
    private readonly CRUDDbContext _databaseDbContext;
    public CountriesService(CRUDDbContext databaseDbContext, bool isInitialize = true)
    {
    ...
    public CountryResponse AddCountry(CountryAddRequest? countryAddRequest)
    {
        ...
        if (_databaseDbContext.Countries.Where(x => x.CountryName == countryAddRequest.CountryName).Count() > 0) { throw new ArgumentException("Given country already exists"); }
        Country country = countryAddRequest.ToCountry();
        country.CountryId = Guid.NewGuid();
        // _countries.Add(country);
        _databaseDbContext.Countries.Add(country);
        _databaseDbContext.SaveChanges();
        return country.ToCountryResponse();
    }
    public List<CountryResponse> GetAllCountries()
    {
        return _databaseDbContext.Countries.Select(x => x.ToCountryResponse()).ToList();
    ...
    public CountryResponse? GetCountryByCountryId(Guid? countryId)
    {
        ...
        Country? country = _databaseDbContext.Countries.FirstOrDefault<Country>(x => x.CountryId == countryId);
